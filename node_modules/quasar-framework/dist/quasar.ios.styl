...
ul.breadcrumb
  ...
  display inline-flex
  flex-wrap nowrap
  margin round($breadcrumb-height / 3) 0 0 0
  padding 0
  i
    font-size 1.7em
  li
    float left
    margin-bottom round($breadcrumb-height / 3)
    a
      color white
      display block
    ...
    a
      &:before, &:after
        ...
        border-width round($breadcrumb-height / 2) 10px
      &:before
...
$checkbox-size = 21px
.q-checkbox-unchecked, .q-checkbox-checked
...
body
  &.cordova.platform-ios, &.demo-site.within-iframe
    .layout-header > .q-toolbar:first-child, .layout-aside.top-padding
