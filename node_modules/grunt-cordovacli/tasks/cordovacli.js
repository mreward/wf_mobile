...
module.exports = function (grunt) {
    ...
    runCordova = function (args, opts, done) {
        ...
        const child = grunt.util.spawn(spawn_cmd,
            (err, result) => {
                if (err) {
                    grunt.log.error(err);
                } else {
    ...
    runCordovaParallel = function (tasks, done) {
        grunt.util.async.parallel(tasks, (err, result) => {
            if (err) {
    ...
    runCordovaSeries = function (tasks, done) {
        grunt.util.async.series(tasks, (err, result) => {
            if (err) {
    ...
    runCreate = function (options, done) {
        ...
        const args = ['create', options.path, options.id, options.name].concat(options.args);
        runCordova(args, {}, done);
    };

    isPlatformExists = function (p, cordovaRootPath) {
        let platform_name;
        let platform_cdv_dir;
        let platform_src_dir;
        let pkg;
    ...
    runPlugin = function (options, done) {
        ...
        options.plugins.forEach((p) => {
            ...
            f = function (callback) {
                runCordova(['plugin', options.action, p.name || p].concat(p.args || options.args), { cwd: options.path }, callback);
            };
            tasks.push(f);
        });
        runCordovaSeries(tasks, done);
    ...
    runFullCycle = function (commands, options, done) {
        ...
        commands.forEach((command) => {
            if (command === 'create') {
                tasks.push((cb) => { runCreate(options, cb); });
            } else if (command === 'platform') {
                options.action = 'add';
                tasks.push((cb) => { runPlatform(options, cb); });
            } else if (command === 'plugin' && options.plugins) {
                options.action = 'add';
                tasks.push((cb) => { runPlugin(options, cb); });
            } else if (command === 'prepare' || command === 'compile' || command === 'build') {
                tasks.push((cb) => { runCordova([command].concat(options.args), { cwd: options.path }, cb); });
            }
        });
        runCordovaSeries(tasks, done);
    };
